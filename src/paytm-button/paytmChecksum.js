...
class PaytmChecksum {
  static encrypt(input, key) {
    let cipher = crypto.createCipheriv("AES-128-CBC", key, PaytmChecksum.iv);
  ...
  static async generateSignatureByString(params, key) {
    let salt = await PaytmChecksum.generateRandomString(4);
    return PaytmChecksum.calculateChecksum(params, key, salt);
  }

  static verifySignatureByString(params, key, checksum) {
    let paytm_hash = PaytmChecksum.decrypt(checksum, key);
  ...
  static calculateChecksum(params, key, salt) {
    let hashString = PaytmChecksum.calculateHash(params, salt);
